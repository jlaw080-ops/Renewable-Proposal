...
(function () {
  ...
  var SYSTEM_PROMPT =
    ...
    "- 태양광 비중이 큰 조합은 에너지자립률(ECO2)이 높아 ZEB 등급 향상·인센티브(용적률 완화 등) 확보에 유리함을, 해당 특성이 표시된 경우 반드시 언급할 것.";
  ...
  function buildExplainMessage(card, ctx) {
    ...
    // 태양광 합계 최대 조합 — 높은 에너지자립률(ECO2)→ZEB 인센티브 특성 강조(필수 반영)
    if (card.최대태양광) {
      L.push("\n## 태양광 자립률 특성 (필수 반영)");
      L.push("- 이 조합은 표시된 조합 중 태양광 합계 용량이 가장 큽니다 (약 " + Math.round(card.태양광합계 || 0).toLocaleString() + " kW).");
      L.push("- 태양광 비중이 높아 에너지자립률(ECO2 기준)이 상대적으로 높고, 이는 ZEB 등급 향상과 용적률 완화 등 인센티브 확보에 유리합니다.");
      L.push("- 위 특성(높은 태양광 자립률 → ZEB 인센티브 유리)을 설명에 반드시 포함하세요.");
    }

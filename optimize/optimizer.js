...
(function () {
  ...
  function optimizeCore(ctx) {
    ...
    // [5] 표시 필터 — 「예외 조합」(단일 형식 100% + 저적합)만 화면 표시(ranked)에서 제외(평가는 유지).
    //   혼합(용량비율 조정) 조합은 적합도와 무관하게 표시 대상 → 적합설비 조합이 상위로 노출된다.
    var 표시하한 = 적합표시하한(ctx);
    var 표시대상 = feasible.filter(function (f) { return !is예외조합(f, 표시하한); });
